...
addpath ../geom
addpath ../main
% addpath C:\Program Files\MATLAB\R2013a\toolbox\SuiteSparse
penalty=1e20;I=zeros(100000000,1);J=I;K=I;idx=0;
...
if fixed_freedoms >0
    for i=1:fixed_freedoms
        ...
        no=nf(sense,node);
        idx=idx+1;
        I(idx)=no;J(idx)=no;K(idx)=penalty;
        loads(no)=penalty*value;
...
fclose(fid);
NN=idx;
fid=fopen('result_sparse.dat','wt');
% !-----------------------equation solution---------------------------------
% kv=sparin(kv,kdiag);
% loads = spabac(kv,loads,kdiag);
KM=sparse(I(1:N,:),J(1:N,:),K(1:N,:),neq,neq);
KM2=sparse(I(1:idx,:),J(1:idx,:),K(1:idx,:),neq,neq);
loads = cholmod2(KM2,loads);loads(abs(loads)<1e-10)=0;
disp=zeros(nn,nodof);
if ndim==2
