function [ coord,num] = hexahedron_xz(iel,x_coords,y_coords,z_coords,nod)
...
% ! 8, 14 or 20-node "bricks" counting x-z planes in the y-direction.
 ...
 num=zeros(nod,1);coord=zeros(nod,3);
 iq=floor((iel-1)/(nxe*nze))+1;
 iplane=iel-(iq-1)*nxe*nze;
 is=floor((iplane-1)/nxe)+1 ;
 ip=iplane-(is-1)*nxe;
switch nod
    case 8
    ...
    otherwise
        disp('Wrong number of nodes for hexahedral element');
end
end
